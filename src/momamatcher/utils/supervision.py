...
def spvs_coarse(data, config):
    ...
    scale = config["MOMAMATCHER"]["RESOLUTION"][0]
...
def get_scale_gt_matrix5(
    ...
    valid_mask0, w_pt0_i, depth_mask0, consistent_mask0 = warp_kpts(
        grid_pt0_i,
        data["depth0"],
        data["depth1"],
        data["T_0to1"],
    ...
    valid_mask1, w_pt1_i, depth_mask1, consistent_mask1 = warp_kpts(
        grid_pt1_i,
        data["depth1"],
        data["depth0"],
        data["T_1to0"],
    ...
    if (valid_mask0 * covisible_mask0 * consistent_mask0 == 0).all() and (valid_mask1 * covisible_mask1 * consistent_mask1 == 0).all():
    ...
    for bs in range(N):
        ...
        geod_mask0 = (
            symmetric_epipolar_distance( bs_grid_pt0, bs_w_pt0, E_0to1[bs], data["K0"][bs], data["K1"][bs] ) <= obj_geod_th
        )
        geod_mask1 = (
            symmetric_epipolar_distance( bs_w_pt1, bs_grid_pt1, E_0to1[bs], data["K0"][bs], data["K1"][bs] ) <= obj_geod_th
        )
    ...
    if require_depth_mask:
        depth_mask0 = torch.zeros_like(depth_mask0).bool()
        depth_mask0[f_b_ids0, f_i_ids0] = True
        depth_mask1 = torch.zeros_like(depth_mask1).bool()
...
def spvs_instance_gt2(data, config):
    ...
    N, _, H0, W0 = data["image0"].shape
    _, _, H1, W1 = data["image1"].shape
    scale_l0, scale_l1, scale_l2 = config["MOMAMATCHER"]["RESOLUTION"]  # 32, 8, 2
    ...
    kpts_mask0_l2 = rearrange(kpts_mask0_l2, "n (h w) -> n h w", h=h0_l2, w=w0_l2)
    kpts_mask1_l2 = rearrange(kpts_mask1_l2, "n (h w) -> n h w", h=h1_l2, w=w1_l2)
    kpts_mask0_l1 = (
        rearrange(
            kpts_mask0_l2, "n (s_h h) (s_w w) -> n s_h s_w (h w)", s_h=h0_l1, s_w=w0_l1
        ).sum(dim=-1)
        > 0
    ) 
    kpts_mask1_l1 = (
        rearrange(
            kpts_mask1_l2, "n (s_h h) (s_w w) -> n s_h s_w (h w)", s_h=h1_l1, s_w=w1_l1
        ).sum(dim=-1)
        > 0
    )
    ...
    class_matrix_l1_gt0 = kpts_mask0_l1.float()
    class_matrix_l1_gt1 = kpts_mask1_l1.float()

    # class matrix l0
    class_matrix_l0_gt0 = torch.zeros((N, h0_l0, w0_l0), device=device)  # 1 for positive  c0 background c1 front
    class_matrix_l0_gt1 = torch.zeros((N, h1_l0, w1_l0), device=device)

    dnum_matrix_l0_gt0 = ((rearrange(class_matrix_l1_gt0, "n (s_h h) (s_w w) -> n s_h s_w (h w)", s_h=h0_l0, s_w=w0_l0) > 0).sum(dim=3).float())
    dnum_matrix_l0_gt0 = dnum_matrix_l0_gt0 / ((h0_l1 * w0_l1) / (h0_l0 * w0_l0))
    
    class_matrix_l0_gt0[torch.nonzero(dnum_matrix_l0_gt0 > 0.1, as_tuple=True)] = 1  # [N, h0_l0, w0_l0] # 0.2
    class_matrix_l0_gt0 = class_matrix_l0_gt0.unsqueeze(1)  # [N, 1, h0_l0, w0_l0]
    class_matrix_l0_gt0 = torch.cat([1 - class_matrix_l0_gt0, class_matrix_l0_gt0], dim=1)  # [N, 2, h0_l0, w0_l0]
    class_b0_l0_ids, class_k0_l0_ids = torch.nonzero(rearrange(class_matrix_l0_gt0[:, 1, :, :], "n h w -> n (h w)"), as_tuple=True)

    dnum_matrix_l0_gt1 = ((rearrange(class_matrix_l1_gt1, "n (s_h h) (s_w w) -> n s_h s_w (h w)", s_h=h1_l0, s_w=w1_l0)>0).sum(dim=3).float())
    dnum_matrix_l0_gt1 = dnum_matrix_l0_gt1 / ((h1_l1 * w1_l1) / (h1_l0 * w1_l0))
    class_matrix_l0_gt1[torch.nonzero(dnum_matrix_l0_gt1 > 0.1, as_tuple=True)] = 1  # [N, h1_l0, w1_l0] # 0.2
    class_matrix_l0_gt1 = class_matrix_l0_gt1.unsqueeze(1)  # [N, 1, h1_l0, w1_l0]
    class_matrix_l0_gt1 = torch.cat([1 - class_matrix_l0_gt1, class_matrix_l0_gt1], dim=1)  # [N, 2, h1_l0, w1_l0]
    class_b1_l0_ids, class_k1_l0_ids = torch.nonzero(rearrange(class_matrix_l0_gt1[:, 1, :, :], "n h w -> n (h w)"), as_tuple=True)
...
def spvs_fine(data, config):
    ...
    scale_l0, scale_l1, scale_l2 = config["MOMAMATCHER"]["RESOLUTION"]  # 64, 8, 2
    radius = config["MOMAMATCHER"]["FINE_WINDOW_SIZE"] // 2
