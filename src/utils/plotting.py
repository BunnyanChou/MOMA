...
def _make_evaluation_figure_offset(data, b_id, alpha='dynamic',side=''):
    ...
    for layer_index in range(layer_num):
        ...
        # kpts0 = data['gt_offset_kpts0_f'+side].cpu().numpy()
        # kpts1 = data['gt_offset_kpts1_f'+side].cpu().numpy()
        ...
        color = error_colormap(epi_errs, conf_thr, alpha=alpha)
        if alpha == 'dynamic':
            alpha = dynamic_alpha(kpts0.shape[0])
        # import torch  
        # color = error_colormap(torch.ones(kpts0.shape[0]), 1.5, alpha)
