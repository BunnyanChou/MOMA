...
from src.momamatcher.utils.geometry import pose2fundamental
from src.utils.comm import reduce_mean

class MOMAMatcherLoss(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.count = 0
        self.scale_l0, self.scale_l1, self.scale_l2 = config['momamatcher'][
            'resolution']  # 64, 8, 2
    ...
    def forward(self, data):
        ...
        cas_score0 = data['pred_class0_l1']  # [batch, 1, h0_l1, w0_l1]
        cas_score1 = data['pred_class1_l1'] # [batch, 1, h1_l1, w1_l1]
        
        
        gt_cas0 = data['spv_class_l1_gt0']  # [batch, h0_l1, w0_l1]
        gt_cas1 = data['spv_class_l1_gt1']  # [batch, h1_l1, w1_l1]
        ...
        if len(class_k0_l0_ids) != 0 and len(class_k1_l0_ids) != 0:
            ...
            # coarse_loss = self.class_weight * cas_loss + mask_loss
            coarse_loss = self.mask_weight * mask_loss
        else:
            coarse_loss = None
        ...
        loss_flow = self.compute_flow_loss(coarse_corr,data['predict_flow'],\
                                            data['hw0_d8'][0],data['hw0_d8'][1],data['hw1_d8'][0],data['hw1_d8'][1])
        ...
        if coarse_loss is not None:
            ...
            loss_scalars.update({
                ...
                'c': (mask_focal_loss.clone().detach().cpu() +
                      mask_focal_loss.clone().detach().cpu()) / 2,
                'f':
            ...
            loss = mask_focal_loss  # + data['dummy_loss']  # .clone()
            loss_scalars = {
                'mf': mask_focal_loss.clone().detach().cpu(),
                'c': mask_focal_loss.clone().detach().cpu(),
                'f': mask_focal_loss.clone().detach().cpu(),
                'v_n': self.fine_v_num,
                # 'pv_n': self.fine_pv_num,
